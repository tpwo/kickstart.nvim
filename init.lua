...
require('lazy').setup({
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      ...
      vim.keymap.set(
        ...
        require_git_repo(function()
          builtin.git_commits {
            git_command = { 'git', 'log', '--pretty=format:%h %<(15,trunc)%an %<(15,trunc)%ar %s' },
            entry_maker = function(entry)
              ...
              return {
                value = hash,
                display = entry,
                ordinal = vim.fn.system('git log -1 ' .. hash .. ' --pretty=format:%h %s %an %ar'):gsub('\n', ' '),
              }
            end,
          }
        end),
  ...
  { -- Highlight, edit, and navigate code
    ...
    opts = {
      ensure_installed = { 'bash', 'c', 'diff', 'html', 'lua', 'luadoc', 'markdown', 'markdown_inline', 'query', 'vim', 'vimdoc', 'python', 'toml' },
